...
export async function POST(req: Request) {
  ...
  // an advisor cannot use this endpoint to probe claim IDs belonging to
  // another advisor's clients — if the row isn't visible to them under RLS,
  // it isn't visible here either, and we never even reach their Gmail token.
  ...
  const { data: claim, error: claimErr } = await scopedSupabase
    .from('claims')
    .select('id')
    .eq('id', claimId)
    .maybeSingle()
  ...
  // Audit log — who searched, which claim, which terms, how many results.
  // Never logs a subject line, sender, or any message content.
