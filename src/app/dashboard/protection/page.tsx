...
function CoverageChart({title,eyebrow,needLabel,haveLabel,data,accentColor}:{
  ...
  const fmtFull=(n:number)=>n>=1e6?`$${(n/1e6).toFixed(2)}M`:n>=1e3?`$${Math.round(n/1e3)}K`:`$${Math.round(n).toLocaleString()}`
  ...
  function handleMouseMove(e: React.MouseEvent<SVGSVGElement>) {
    ...
    const relX = svgX - PL
    if (relX < 0 || relX > iW) { setHovered(null); return }
    const ageFloat = minA + (relX / iW) * aR
    const idx = Math.min(data.length-1, Math.max(0, Math.round(ageFloat - minA)))
    const d = data[idx]
    setHovered({ ...d, x: PL + xP(d.age), y: PT + yP(d.need) })
  }

  return (
    <div style={{background:'white',border:'0.5px solid var(--line)',padding:'20px 22px',position:'relative'}}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{display:'block',overflow:'visible',cursor:'crosshair'}}
        ...
        {hovered && (<>
          <line x1={hovered.x} y1={PT} x2={hovered.x} y2={PT+iH} stroke="rgba(0,0,0,0.15)" strokeWidth="0.5" strokeDasharray="3,3"/>
          <circle cx={hovered.x} cy={PT+yP(hovered.need)} r="3" fill="#00C9A7"/>
          <circle cx={hovered.x} cy={PT+yP(hovered.have)} r="3" fill={accentColor}/>
        </>)}
      </svg>
      {hovered && (
        <div style={{position:'absolute',top:56,left:`calc(${((hovered.x-PL)/iW)*100}% - 60px)`,background:'white',border:'0.5px solid var(--line)',padding:'8px 12px',fontSize:11,lineHeight:1.8,pointerEvents:'none',boxShadow:'0 4px 12px rgba(0,0,0,0.08)',zIndex:10,minWidth:120}}>
          <div style={{fontWeight:600,color:'var(--ink)',marginBottom:4}}>Age {hovered.age}</div>
          <div style={{color:'#00C9A7'}}>Need: {fmtFull(hovered.need)}</div>
          <div style={{color:accentColor}}>Have: {fmtFull(hovered.have)}</div>
          {hovered.need > hovered.have && <div style={{color:'#C0392B',fontWeight:500}}>Gap: {fmtFull(hovered.need - hovered.have)}</div>}
        </div>
      )}
